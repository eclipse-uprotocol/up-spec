...
--
To establish an MQTT connection to a broker it *MUST* be possible for a UTransport to configure the `cleanSession` and `sessionExpiry` with values according to the MQTT specification. 

Those values have some important consequences and the following table summarize them shortly:

[cols="1,1,1"]
|===
| cleanSession | sessionExpiry | Note
| true | 0 | Previous session data is deleted and no persistent data (messages, subscribtions) if UTransport gets disconnected
| true | t | Previous session data is deleted and new session will buffer data for time `t`
| false | t | Previous session data (subscribtions, messages) resumed and resumed session will buffer data for time `t`
|===
