...
UUri should be able to be serialized it into micro form.
The mapping from micro-from UUri to Zenoh key is `Vec<u8>` to lower hex string.

Zenoh key for uProtocol MUST start with `up/`.
If UAuthority is defined in UUri, it should be put in the following part.
...
Note that if the UUri only contains UAthority with the empty UEntity and UResource,
