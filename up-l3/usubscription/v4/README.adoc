...
uSubscription service *MUST* notify (send a Notification message to) every susbcription holder on changes to subscription `status`, `expiration` time or `sample_period_ms` of their subscription.
...
Subscription change messages *MUST* use resource ID `0x8000` in the Notification source UURI.
 
=== Custom notifications

Any uEntity may register with uSubscription service to explicitly be sent subscription change Notifications when the subscription `status`, `expiration` time or `sample_period_ms` of an indicated topic (or set of topics) changes. For instance, a `uStreamer` application might be interested in all subscription events involving a topic with an `authority` pointing to the remote network that the `uStreamer` is bridging to.
