...
****
When the `Reset()` function is called, uSubscription service *MUST* notify all subscribers and uEntities that registered for change notifications:

* after flushing stored subscription information (so that subsequent (re)subscribe requests do not get lost with the reset)
* by sending messages of type `Update` for each subscribed/registered topic, with a subscription state of `UNSUBSCRIBED`
****
