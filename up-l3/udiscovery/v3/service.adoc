...
- *Client APIs (link:../../../up-core-api/uprotocol/core/udiscovery/v3/udiscovery.proto[udiscovery.proto]):* Used by uEs to find services and their topic metadata, and for recursive lookups at teh domain and central levels.

- *Replication API (link:../../../up-core-api/uprotocol/core/udiscovery/v3/udiscovery_replicator.proto[udiscovery_replicator.proto]):* Used Internally by uDiscovery services to add, update, or delete service metadata from local, domain, and/or central service instances.
...
`FindServices()` is used to find the authority(ies), instance(s), and version(s) of the serice id (ue_id). The API is passed `FindServicesRequest` message and it returns a `FindServicesResponse` message that contains a batch of 1 or more services that match the search criteria.
...
`FindServiceTopics()` is used to fetch metadata about one or more topic based on the passed `UUri`. Clients call this API to know more metadata about the topic(s) such as the format, id, permission level, etc... The API is passed `GetServiceTopicsRequest` message and it returns a `GetServiceTopicsResponse` message that contains a repeated list of 1 or more `ServiceTopic` messages.
...
--
* link:../languages.adoc[uProtocol Language Libraries] *MUST NOT* implement the client-side of xref:../../../up-core-api/uprotocol/core/udiscovery/v3/udiscovery_replicator.proto[udiscovery_replicator.proto] API as these APIs are only used internally by uDiscovery business logic.
--
...
It is possible the information in domain or central instances might become out of sync with what isa stored in the other instances. In order to rectify this situation:

[.specitem,oft-sid="dsn~discovery-data-reconciliation~1",oft-needs="impl,test"]
--
* Domain and central instances *MUST* ensure that the information within its database is accurate and up to date and reconcile any differences periodically using the client and replication APIs described in this specification.
--

[.specitem,oft-sid="dsn~discovery-data-reconciliation-frequency~1",oft-needs="impl,test"]
--
* *MUST* provide an implementation of data reconciliation using the above APIs where the frequency of reconciliation is customizable so the deployment can adjust the frequency based on the deployment needs.
--
