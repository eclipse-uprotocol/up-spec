...
uTwin "MUST* reject identical copies of an already cached Topic CE, enabling uBus to refrain from forwarding duplicate publish messages to subscribers.
...
When uTwin cache scrubbing is supported, this *MUST* be implemented so as to prevent it from adding latency to critical path execution of active uTwin SetLastMessage transactions, based upon a (TBC 95% full) threshold.  

If required, MVP uTwin cache scrubbing *MUST* remove the oldest (longest gap since last updated) topics in the queue, bringing cache size from 95% to 90% of available capacity.

Post-MVP cache scrubbing *SHOULD* allow for protection of higher priority content, in order to enavvle it to be prevented from being removed, along with determination of "staleness" for specific topics/topic classes/priorities.
