...
In the event that limited cache capacity is available for uTwin and the potential exists for it to run out of capacity if all or a significant portion of available Topics provided by or used by on-device uEs, uTwin *SHOULD* actively manage its cache to ensure that Topics with the longest interval since they were last updated, potentially also based upon specific (TBC) priority critria are removed from the cache in order to ensure that new content can continue to be accepted.  
...
If required, MVP uTwin cache scrubbing *MUST* remove the oldest (longest gap since last updated) topics in the queue, bringing cache size from (TBC) 98% to 90% of available capacity.
