...
A call to SetLasMessage (typically from uBus) to update the uTwin internal cache with a message for a given topic (UMessage.source). Return value status.Code is one of:
