...
* when running without persistent storage, the cache *MUST* be initialized to an empty condition
* when running with persistent storage (future option), the cache *SHOULD* be initialized to the last saved state for each Topic.
...
MVP uTwin cache scrubbing *MUST* remove the oldest (longest gap since last updated) topics in the queue, bringing cache size from (TBC) 98% to 90% of available capacity.
