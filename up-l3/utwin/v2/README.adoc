...
uTwin is intended to leverage a RAM-based cache.   On startup, the uTwin cache *MUST* be initialized to an empty condition.

uTwin *MUST* indicate when its capacity is exhausted and it is unable to accept a new / updated SetLastMessage command.

uTwin *MUST* reject identical copies of an already cached Topic CE (identical CE message ID or Topic URI and Timestamp), enabling uBus to refrain from forwarding duplicate publish messages to subscribers.

When a specified Cache capacity is defined for uTwin, and the potential exists for it to run out of capacity if all or a significant portion of available Topics provided by or used by on-device uEs, uTwin *MUST* actively manage its cache to ensure that Topics with the longest interval since they were last updated are the first to be removed, while also allowing for a priority-based means of protecting critical topics from scrubbing.

When uTwin cache scrubbing is supported, this *MUST* be implemented so as to prevent it from adding latency to critical path execution of active uTwin SetLastMessage transactions, when triggered.  

MVP uTwin cache scrubbing *MUST* remove the oldest (longest gap since added/last updated) topics in the queue, bringing cache size from (TBC) 98% to 90% of available capacity.

Post-MVP cache scrubbing *SHOULD* allow for protection of higher priority content per UAttributes.priority, in order to enable it to be prevented from being removed.  This *SHOULD* be managed by a uTwin property, specifying the threshold message Priority level from and below which, based upon increasing criticality with lower ordinate priority level, messages cannot be deleted when scrubbing due to capacity limits.

uTwin cache size allocation *SHOULD* ensure that it supports not less than 125% of the capacity for all IV Topics, to allow for a percentage of published traffic coming from the cloud.
...
* uTwin has restarted and its internal cache that stores the last events has been cleared

The implementation of `GetLastMessages()` *MUST* enforce permissions associated with each Topic being requested.  
...
Each device *MUST* have its own instance of uTwin. 

uTwins *MUST* store the last published event for all topics (local and remote) published to the device.
...
One uTwin instance per device is created. The uBus of each device is responsible to feed the uTwin database by calling `SetLastMessage()` for each *incoming* CE with `type=pub` that it routes, therefore achieving the desired behavior of a creating a cache of all received Publish events on the device.
