...
uTwin *MUST* indicate when its capacity is exhausted and it is unable to accept a new / updated SetLastEvent command.

In the event that limited cache capacity is available for uTwin and the potential exists for it to run out of capacity if all or a significant portion of available Topics provided by or used by on-device uEs, uTwin *SHOULD* actively manage its cache to ensure that the oldest and lowest priority Topics (subject to implementation-specific constraints) are removed from the cache in order to ensure that new content can continue to be accepted.  

If uTwin cache scrubbing is supported, this *MUST* be implemented so as to prevent it from adding latency to active uTwin SetLastEvent transactions, potentially based upon a configurable threshold (e.g. 95% full).  Criteria for protection of higher priority content will need to be established, along with determination of "staleness" for specific topics/topic classes.
...
The implementation of `GetLastEvents()` *MUST* enforce permissions associated with the Topic being requested
...
A uTwin instance *MUST* run on each uDevice. 

Each uTwin instance *MUST* store the last published event on each Topic it 'manages', including event received by Streamers (therefore remotely published)  and published to 'local' subscribers.
...
* *The requested Topic is already subscribed by another uE running on the same device:*
  ** The last published event has already been stored in the uTwin instance for the device, and as a result, can simply be returned when requested by `GetLastEvents()`
