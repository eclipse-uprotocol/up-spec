...
In uProtocol, uEs rely on other uEs that expose services, named uServices. These uServices deliver information as events through the uPlatform. These events can be considered to represent updates to the uService _state._ The collection of events that a uService publishes therefore represents the current "public state" of the uService (public because some more internal states might exist but not published).

An event published on a uDevice is propagated to all other uDevices that have at least one uE subscribed to the corresponding Topic, but only to those devices, These events can therefore be stored on the received devices "for free", i.e. without requiring a dedicated transmission. Storing the last event received per-topic, on each uDevice, therefore enables access without re-transmission overhead/cost/latency for uEs running on the same device devices that wish to retrieve the last state of a uService, i.e. the last event published on a Topic. Implementing such storage mechanism essentially implements a digital twin mechanism for uEs in the sense that uE's last known states are stored in all uDevice that are subscribed to the uService, and accessible by local uE subscribed to Topics of the uService. <<utwin-overview>> figure below provides a visual description of the context.

.uTwin
[#utwin-overview]
image::utwin.drawio.svg[uTwin]

A uE relying on a uService will therefore subscribe to Topics created by the uService and that it uses to publish its updated "state". Consequently, once a uE is _connected_ to uPlatform *and* _subscribed to uServices it requires_, it will receive any state update (of interest, i.e. to which it subscribed). The unresolved issue is to define a mechanism that enables a uE connecting to uPlatform to receive the "last known state" of the uService it relies on. A uE _connects_ to uPlatform for the following reasons:
...
So, for example, if a cloud application MyApp subscribes to the tire pressure of a given vehicle (named vehA), then every time vehA publishes a new value for the tire pressure, the event will be delivered to the cloud and deliver it to MyApp. The uTwin instance running on the cloud will also receive this information and store it with the primary key including the vehicle name vehA and the topic name TirePressure, so that any other software wishing to subsequently access the tire pressure can simply request it from the cloud uTwin.
...
WARNING: Usage of this function should be restricted to uPlatform components, specifically the uBus
...
In some cases, a uE might require one-time access to a Topic, that is, to receive only the last event published on a Topic, without receiving future event that will be published. uTwin can be used for this purpose. There are 4 possible cases in this scenario:
