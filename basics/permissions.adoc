...
Permissions are explicitly requested by client entities in their platform specific deployment manifests (i.e. Android package manifests), the requests represented by customary uProtocol URIs. A CAP is applicable to every uPA to which the corresponding URI (including generic ones) can be resolved.

* Permission *MUST* be granted to a client _if and only if_ its uPL is less or equal the uPL set for the uPA
...
* The runtime enforcement of CAPs *MUST* be done by uSubscription service (for subscriptions to topics) or by the uPlatform dispatchers (ex. uBus) in case of RPCs.

CAP assumes that the link:../up-l1/README.adoc[uTransport] implementation allows for secure determination of the client’s operational context to verify its identity and claimed uPL.
