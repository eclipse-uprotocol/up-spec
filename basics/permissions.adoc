...
Claimed level of a uE is also handled by xref:../up-l1/README.adoc[uTransport]. Implementations will determine the claimed level of the caller in a secure manner, and this will also be done differently depending on the xref:../up-l1/README.adoc[uTransport] implementation.

When the calling uE permission level needs to be carried across different uTransport implementations, or across hops (i.e. across dispatchers such as the uBus), the callers permission level is placed in link:../up-core-api/uprotocol/v1/uattributes.proto[uAttributes `permission_level`].
